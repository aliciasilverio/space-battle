...
Today you will be using your knowledge of OOP, loops, and functions to build a rudimentary **space battle** game. The game will be programmed for, and played in the terminal (using the prompt-sync package install via `npm i prompt-sync`). You will need to use said package to get user input. Refer back to the choose your own adventure code for a starter set up.
...
Make a loop that generates alien ships. Push those constructed objects into a predefined array. Start with 6 ships (the loop should run 6 times).
